...
block = s[start:end]
old_start = block.index('    <style>')
old_end = block.index('    </style>', old_start) + len('    </style>')
new_css = '''    <style>
      ...
      .setta-login-heading {{ text-align:center !important; padding:0 22px !important; }}
...
block = block[:old_start] + new_css + block[old_end:]
s = s[:start] + block + s[end:]
p.write_text(s, encoding='utf-8')
print('Login convertido para um unico card centralizado.')

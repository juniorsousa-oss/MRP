...
def load_sources(cb, eb, gb, pb, mb):
    ...
    rg['Semana válida'] = rg['Semana'].str.match(r'^\d{4}-\d{2}
...
tc_rows = mt[(mt['MATERIAL'] > 0) & (mt['SEMANA DE NECESSIDADE'].str.match(r'^\d{4}-\d{2}].copy()
...
pc_week = cp[(cp['QUANTIDADE P.C'] > 0) & (cp['SEMANA DE ATENDIMENTO P.C'].str.match(r'^\d{4}-\d{2}].groupby(['CÓD','SEMANA DE ATENDIMENTO P.C'])['QUANTIDADE P.C'].sum()
...
sc_week = cp[(cp['QUANTIDADE S.C'] > 0) & (cp['SEMANA DE ATENDIMENTO S.C'].str.match(r'^\d{4}-\d{2}].groupby(['CÓD','SEMANA DE ATENDIMENTO S.C'])['QUANTIDADE S.C'].sum()
sc_week.index.names = ['Código','Semana']
sc_dated = sc_week.groupby(level=0).sum()

# FABRICAÇÃO: B + E; cada OP distinta = 1 peça.
op = mt[(mt['CÓDIGO PRODUTO'] > 0) & (mt['SEMANA DE ENTREGA'].str.match(r'^\d{4}-\d{2}][['ORDEM DE PRODUÇÃO','CÓDIGO PRODUTO','SEMANA DE ENTREGA']].drop_duplicates()
...
bio.seek(0)
st.download_button('Exportar conferência do MRP (.xlsx)',bio.getvalue(),'MRP_Conferencia.xlsx','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
, na=False))].groupby(['CÓD','SEMANA DE ATENDIMENTO P.C'])['QUANTIDADE P.C'].sum()
pc_week.index.names = ['Código','Semana']
pc_total = cp[cp['QUANTIDADE P.C'] > 0].groupby('CÓD')['QUANTIDADE P.C'].sum()
...
bio.seek(0)
st.download_button('Exportar conferência do MRP (.xlsx)',bio.getvalue(),'MRP_Conferencia.xlsx','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
, na=False))].groupby(['CÓD','SEMANA DE ATENDIMENTO S.C'])['QUANTIDADE S.C'].sum()
sc_week.index.names = ['Código','Semana']
sc_dated = sc_week.groupby(level=0).sum()
...
bio.seek(0)
st.download_button('Exportar conferência do MRP (.xlsx)',bio.getvalue(),'MRP_Conferencia.xlsx','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
, na=False))][['ORDEM DE PRODUÇÃO','CÓDIGO PRODUTO','SEMANA DE ENTREGA']].drop_duplicates()
fab_week = op.groupby(['CÓDIGO PRODUTO','SEMANA DE ENTREGA']).size().astype(float)
fab_week.index.names = ['Código','Semana']
fab_total = fab_week.groupby(level=0).sum()

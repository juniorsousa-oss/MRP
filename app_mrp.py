...
st.set_page_config(page_title="MRP | SETTA", page_icon="assets/mrp_setta_icon.png", layout="wide")
...
def _require_login(login_cfg):
    ...
    <style>
      ...
      div[data-testid="stForm"] button {{
        ...
        height:29px !important; min-height:29px !important; margin-top:9px !important; border-radius:7px !important;
...
ultima_solicitacao=rg.groupby(["Projeto","Código"],as_index=False)["Data Solicitação"].max().rename(columns={"Código":"Produto","Data Solicitação":"Última Solicitação"}); ultima_solicitacao["Última Solicitação"]=ultima_solicitacao["Última Solicitação"].apply(formatar_data_br)
data_cm_projeto=rg.groupby(["Projeto","Código"],as_index=False)["Data CM"].max().rename(columns={"Código":"Produto","Data CM":"Data CM"}); data_cm_projeto["Data CM"]=data_cm_projeto["Data CM"].apply(lambda x:formatar_data_br(x) if pd.notna(pd.to_datetime(x,errors="coerce")) else "NI")
demanda_projeto_base=rg_mrp[(~rg_mrp["Código"].isin(codigos_ii))&rg_mrp["Pendência"].ne(0)].copy(); demanda_projeto_base=demanda_projeto_base[["Projeto","Código","Pendência","Semana"]].rename(columns={"Código":"Produto","Pendência":"Necessidade","Semana":"Semana de Necessidade"}); demanda_projeto_base=demanda_projeto_base.merge(cad[["Código","Descrição"]].rename(columns={"Código":"Produto"}),on="Produto",how="left"); demanda_projeto_base=demanda_projeto_base.merge(ultima_solicitacao,on=["Projeto","Produto"],how="left"); demanda_projeto_base=demanda_projeto_base.merge(data_cm_projeto,on=["Projeto","Produto"],how="left"); demanda_projeto_base["Data CM"]=demanda_projeto_base["Data CM"].fillna("NI"); demanda_projeto_base["Última Solicitação"]=demanda_projeto_base["Última Solicitação"].fillna("NI"); demanda_projeto_base=demanda_projeto_base.sort_values(["Produto","Semana de Necessidade","Projeto"]).reset_index(drop=True)

...
def load_sources(cb,eb,gb,pb,mb):
    ...
    gr=pd.read_excel(BytesIO(gb),sheet_name="RelatorioTratado"); rg=pd.DataFrame({"Código":num(gr.iloc[:,2]),"Pendência":num(gr.iloc[:,6]).fillna(0),"Data Solicitação":pd.to_datetime(gr.iloc[:,3],errors="coerce",dayfirst=True),"Data CM":pd.to_datetime(gr.iloc[:,12],errors="coerce",dayfirst=True),"Semana":num(gr.iloc[:,13]),"Projeto":gr.iloc[:,1].fillna("").astype(str)}).dropna(subset=["Código"]); rg["Código"]=rg["Código"].astype("int64"); rg_mrp=rg[rg["Semana"].notna()&rg["Semana"].between(1,53)].copy(); rg_mrp["Semana"]=rg_mrp["Semana"].astype("int64")
...
sa_week=rg_mrp.groupby(["Código","Semana"],as_index=False)["Pendência"].sum().rename(columns={"Pendência":"Demanda S.A."}); tc_rows=mt[(mt["Material"]>0)&mt["Semana Necessidade"].between(1,53)].copy(); tc_week=tc_rows.groupby(["Material","Semana Necessidade"],as_index=False)["Quantidade"].sum(); tc_week.columns=["Código","Semana","Demanda TC/TP"]; pc_week=cp[(cp["Quantidade P.C."]>0)&cp["Semana P.C."].between(1,53)].groupby(["Código","Semana.P.C."],as_index=False)["Quantidade P.C."].sum() if False else cp[(cp["Quantidade P.C."]>0)&cp["Semana P.C."].between(1,53)].groupby(["Código","Semana P.C."],as_index=False)["Quantidade P.C."].sum(); pc_week.columns=["Código","Semana","P.C."]; sc_all=cp[cp["Quantidade S.C."]>0].groupby("Código",as_index=False)["Quantidade S.C."].sum().rename(columns={"Quantidade S.C.":"S.C."}); sc_week=cp[(cp["Quantidade S.C."]>0)&cp["Semana S.C."].between(1,53)].groupby(["Código","Semana S.C."],as_index=False)["Quantidade S.C."].sum(); sc_week.columns=["Código","Semana","S.C."]; op=mt[(mt["Código Produto"]>0)&mt["Semana Entrega"].between(1,53)].copy(); fab_week=op.groupby(["Código Produto","Semana Entrega"]).size().reset_index(name="Produzindo"); fab_week.columns=["Código","Semana","Produzindo"]
